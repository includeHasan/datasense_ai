...
import { createSqlSource, type SqlSourceKind } from "../sources/sql-source.js";
import {
  getProfileForOwner,
  registerSource,
...
export function registerSourceRoutes(app: FastifyInstance): void {
  app.post("/sources/file", { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    try {
      ...
      const sourceId = registerSource(source, profile, request.user.id);
      return reply.send({ sourceId, profile });
    } catch (error) {
      return reply.code(400).send({
        error: `Failed to ingest file: ${error instanceof Error ? error.message : String(error)}`,
      });
    }
  });
  ...
  app.post("/sources/db", { preHandler: [app.authenticate] }, async (request, reply) => {
    ...
    try {
      const source = createSqlSource(kind as SqlSourceKind, connectionString);
      const profile = await source.profile();
      const sourceId = registerSource(source, profile, request.user.id);
      return reply.send({ sourceId, profile });
    } catch (error) {
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      const profile = getProfileForOwner(request.params.id, request.user.id);
      if (!profile) {
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      const cached = getSuggestedQuestionsForOwner(id, request.user.id);
      if (cached) {
        return reply.send({ questions: cached });
      }

      try {
        const questions = await suggestQuestions(profile);
        cacheSuggestedQuestionsForOwner(id, request.user.id, questions);
